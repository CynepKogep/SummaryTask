						...
						<table id="list_order_table">
							...
							<c:forEach var="bean" items="${usersList}">
								<tr>
									...
									<td><c:if test="${bean.accessesUsers == 'unlocked'}">
											<fmt:message key="resource_jsp.unlocked" />
										</c:if> <c:if test="${bean.accessesUsers == 'blocked'}">
									...
									<td>
										<form id="make_order" action="controller">
											<c:if
												test="${bean.accessesUsers == 'unlocked' && bean.roles == 'client'}">
												<input type="hidden" name="command" value="listOrdersA" />
												<input type="hidden" name="id_user" value='${bean.id}' />
												<input type="hidden" name="accesses_users"
													value='${bean.accessesUsers}' />
											...
											<c:if
												test="${bean.accessesUsers == 'blocked' && bean.roles == 'client'}">
												<input type="hidden" name="command" value="listOrdersA" />
												<input type="hidden" name="id_user" value='${bean.id}' />
												<input type="hidden" name="accesses_users"
													value='${bean.accessesUsers}' />
