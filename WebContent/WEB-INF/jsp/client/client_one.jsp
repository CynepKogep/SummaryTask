...
<body>
		...
		<tr>
			<td class="content">
				...
				<form id="make_order" action="controller">
					<input type="hidden" name="command" value="listClientCards" /> <input
						type="hidden" name="command_number" value="01_number" />
				...
				<form id="make_order" action="controller">
					<input type="hidden" name="command" value="listClientCards" /> <input
						type="hidden" name="command_number" value="02_name" />
					<c:if test="${localization_value == 'ru'}">
				...
				<form id="make_order" action="controller">
					<input type="hidden" name="command" value="listClientCards" /> <input
						type="hidden" name="command_number" value="03_date_balance" />
						...
						<table id="list_order_table">
							...
							<c:forEach var="bean" items="${cardsList}">
								<tr>
									...
									<td>${bean.name}</td>
									<td>${bean.balance}</td>
									<td><c:if test="${localization_value == 'ru'}">
						                ${bean.lastNameRu}
						            </c:if> <c:if test="${localization_value == 'en'}">
						                ${bean.lastName}
						            </c:if></td>
									<td><c:if test="${localization_value == 'ru'}">
						                ${bean.firstNameRu}
									...
									<td><c:if test="${bean.accessesAccounts == 'unlocked'}">
											<fmt:message key="resource_jsp.unlocked" />
										</c:if> <c:if test="${bean.accessesAccounts == 'blocked'}">
											<fmt:message key="resource_jsp.blocked" />
										</c:if></td>
									<td><c:if test="${bean.nameUnlockRequest == 'exists'}">
											<fmt:message key="resource_jsp.unlock_request" />
										</c:if> <c:if test="${bean.nameUnlockRequest == 'doesnotexist'}">
 ...
 --> <!--  -->
										<form id="make_order" action="controller">
											...
											<c:if
												test="${bean.nameUnlockRequest == 'doesnotexist' && bean.accessesAccounts == 'blocked'}">
												<input type="hidden" name="command" value="listClientCards" />
												<input type="hidden" name="name_button"
													value='unlock_request' />
												<input type="hidden" name="id_card" value='${bean.id}' />
												<input type="hidden" name="accesses_card"
													value='${bean.accessesAccounts}' />
												<input type="hidden" name="unlock_request_card"
													value='${bean.nameUnlockRequest}' />
