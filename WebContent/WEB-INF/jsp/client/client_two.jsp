...
<script type="text/javascript"
	src="jspdf.plugin.standard_fonts_metrics.js"></script>
...
<body>
			...
			<td class="content">
				...
				<div>
					...
					<form style="display: inline-block" id="make_order"
						action="controller">
						<input type="hidden" name="command" value="listClientPays" /> <input
					...
					<form style="display: inline-block" id="make_order"
						action="controller">
						<input type="hidden" name="command" value="listClientPays" /> <input
							type="hidden" name="command_number" value="02_data" />
						<c:if test="${localization_value == 'ru'}">
					...
					<form style="display: inline-block" id="make_order"
						action="controller">
						<input type="hidden" name="command" value="listClientPays" /> <input
							type="hidden" name="command_number" value="03_data_desc" />
						<c:if test="${localization_value == 'ru'}">
					...
					<form style="display: inline-block" id="make_order"
						action="controller">
						<input type="hidden" name="command" value="listClientPays" /> <input
							type="hidden" name="command_number" value="pdg_page" /> <input
							type="hidden" name="name_button" value='pdg_button' /> Path: <input
							name="path" type="text" value="C:\" required
							pattern="^[\\0-9A-Fa-f:.]+$"> <input type="submit"
							value='pdf' />
						<%-- 				    <c:if test = "${localization_value == 'ru'}">
				        <input type="submit" value='по дате (desc)'/>
				    </c:if>
 ...
 --%>
					...
					<button style="display: inline-block"
						onclick="javascript:demoFromHTML();">PDF(js)</button>

					<form style="display: inline-block" id="make_order"
						action="controller">
						<input type="hidden" name="command" value="listClientPays" /> <input
							type="hidden" name="name_button" value="pdf_2" /> <input
							type="submit" value='pdf(jsp)' />
					</form>
											...
											<form id="make_order" action="controller">
												...
												<c:if test="${bean.nameStatusPay == 'prepared'}">
													...
													<input type="hidden" name="credit_account_id"
														value='${bean.creditAccountId}' />
													<input type="hidden" name="pay_sums" value='${bean.sums}' />
													<input type="hidden" name="status_pay_id"
														value='${bean.statusPayId}' />
